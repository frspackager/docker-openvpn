...
Execute the following commands. Note that you might want to change the volume `$PWD` or use a data docker container for this.
    ...
    docker run --net=none --rm -t -i -v $PWD:/etc/openvpn frspackager/openvpn ovpn_genconfig -u udp://VPN.SERVERNAME.COM
    docker run --net=none --rm -t -i -v $PWD:/etc/openvpn frspackager/openvpn ovpn_initpki
    docker run --net=none --rm -t -i -v $PWD:/etc/openvpn frspackager/openvpn ovpn_copy_server_files
...
The following options have been tested successfully:
    ...
    docker run -v $OVPN_DATA:/etc/openvpn --net=none --rm frspackager/openvpn ovpn_genconfig -C 'AES-256-CBC' -a 'SHA384'
...
EasyRSA will generate 4096 bit RSA keys when the `-e EASYRSA_KEY_SIZE=4096` argument is added to `ovpn_initpki` and `easyrsa build-client-full` commands.
    ...
    docker run -e EASYRSA_KEY_SIZE=4096 -v $OVPN_DATA:/etc/openvpn --rm -it frspackager/openvpn ovpn_initpki
    docker run -e EASYRSA_KEY_SIZE=4096 -v $OVPN_DATA:/etc/openvpn --rm -it frspackager/openvpn easyrsa build-client-full CLIENTNAME nopass
...
A simple way to avoid having Docker log output for a given command is to run with `--log-driver=none`, e.g
    ...
    docker run -v $OVPN_DATA:/etc/openvpn --log-driver=none --rm frspackager/openvpn ovpn_getclient USER > USER.ovpn
